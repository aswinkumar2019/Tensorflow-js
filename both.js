class App extends React.Component {
  ...
  detectFromVideoFrame = (model, modelSsd, video) => {
    model.estimateFaces(video,"True").then(predictions => {
      console.log(predictions.length)
      requestAnimationFrame(() => {
        this.detectFromVideoFrame(model, video);
      });
      modelSsd.detect(video).then(predictions => {
  ...
  componentDidMount() {
    if (navigator.mediaDevices.getUserMedia || navigator.mediaDevices.webkitGetUserMedia) {
      ...
      const loadModelPromiseSsd = cocoSsd.load();
      // resolve all the Promises
      Promise.all([loadlModelPromise, loadModelPromiseSsd, webcamPromise])
        .then(values => {
          this.detectFromVideoFrame(values[0], values[1], this.videoRef.current);
        })
        .catch(error => {
          console.error(error);
        });
